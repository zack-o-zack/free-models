      ...
      models: [
        {
          ...
          name: "Beta",
          stale_only: true,
          nested: { stale: "whole record" },
        },
        {
          id: "stealth:inactive",
          name: "Inactive",
          private_field: { must: "remain" },
        },
        {
          id: "acme/alpha",
          name: "Alpha",
          old_generated: "remove me",
        },
      ],
    });
...
describe("canonical metadata refresh", () => {
    ...
    await withWorkspace(async (workspace) => {
      ...
      expect(await Bun.file(join(workspace, "catalogue/canonical-models.json")).json()).toEqual({
        models: [
          ...
          { id: "zeta/beta", name: "Beta", summary: "fresh beta" },
        ],
      });
  ...
  test("applies partial results while retaining missing, inactive, and extra records", async () => {
    await withWorkspace(async (workspace) => {
      ...
      const before = await Bun.file(canonicalPath).json();
      await writeJson(join(workspace, "metadata-fixture.json"), {
        results: {
      ...
      const after = await Bun.file(canonicalPath).json();
      expect(after.models.find((model: { id: string }) => model.id === "acme/alpha")).toEqual({
        id: "acme/alpha",
        name: "Alpha",
        refreshed: true,
      });
      expect(after.models.find((model: { id: string }) => model.id === "zeta/beta")).toEqual(
        before.models.find((model: { id: string }) => model.id === "zeta/beta"),
      );
      expect(after.models.find((model: { id: string }) => model.id === "stealth:inactive")).toEqual(
        before.models.find((model: { id: string }) => model.id === "stealth:inactive"),
      );
      expect(after.models.some((model: { id: string }) => model.id === "unknown/model")).toBe(
        false,
      );
    });
  });
  ...
  test("normalizes equivalent metadata payload key order without file churn", async () => {
    await withWorkspace(async (workspace) => {
      ...
      const firstCanonical = await Bun.file(canonicalPath).text();
